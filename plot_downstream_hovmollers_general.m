%% copied over from NPac_hovmoller.m in Paper1_figures
%C Hersh, 2024
...
%% load in climatology 
if ~exist('clim')
    clim = load('./sig1grid/gridonSig1_climatology_cora_argo_huber25_sla_ug_lscovtar_200.mat');
end 
...
ii = 2; %choose by visually identifying ventilation regions on streamline plots
...
lon_nums(lon_nums > 360) = lon_nums(lon_nums > 360) - 360; %some longitudes are > 360 for some reason 
...
mean_sa_shift = cat(1,mean_sa(11:end,:),mean_sa(1:10,:)); %shift data by 20 longitude
% plot mean argo salinity with streamlines overlaid
[sprc]=prctile(mean_sa_shift(:)',[70,98]);
ax1 = subaxis(1,3,1,'SH',0.02);
cl = [sprc(1)+ [0:0.025:1]*(sprc(2) - sprc(1))];
xi_shift = 20:2:380;
contourf(xi_shift,yi,mean_sa_shift',[20,cl,40],'linecolor','flat') 
hold on
caxis(sprc);
gebco('k')
cmap = cmocean('haline');
...
c.Label.FontSize = 10;
c.FontSize = 10;
title(['{\sigma}_1 = ',num2str(ostrme(ii).sigma1,4)],'FontSize',14);
%axis([110,260,-40,40])
%yticklabels({'40S','30S','20S','10S','0','10N','20N','30N','40N'})
%xticklabels({'120E','140E','160E','180E','160W','140W','120W','100W'})
axis([280,380,-40,40])
yticklabels({'40S','30S','20S','10S','0','10N','20N','30N','40N'})
xticks([280 300 320 340 360 380])
xticklabels({'80W','60W','40W','20W','0','20E'})
...
hold on
xpath_shift = xpath;
xpath_shift(xpath_shift < 20) = xpath_shift(xpath_shift<20) + 360;
%xpath_shift = xpath_shift + 20; %dumb but just trying to get the streamline to plot correctly in the S Atlantic
for dd = 1:length(markdist)
    plot(xpath_shift(markdistidx(dd)),ypath(markdistidx(dd)),'r*','LineWidth',2)
end
plot(xpath_shift(1),ypath(1),'r*','LineWidth',3) %streamline start
plot(xpath_shift,ypath,'r-','LineWidth',2) %ecco streamline
...
plot(advect_linex(1:end-1),2004+rem(advect_liney(1:end-1)-2004,15.5),'k','Linewidth',4);
%xlim([0 6000])
...
title('ECCO','FontSize',12);
hold on
plot(va(1:2),[2004,2004],'k-')
plot(va(1:2),[2016,2016],'k-')

%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/NPac_hovmoller_spice.png')
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/NPac_hovmoller_spice.fig')
...
figure('Position',[10 -100 1600 800])
pvclim_shift = cat(1,pvclim(11:end,:),pvclim(1:10,:)); %shift data by 20 longitude

ax1 = subaxis(1,3,1,'SH',0.02);
[sprc]=prctile(abs(pvclim_shift(:)'),[5,95]);
cl = [sprc(1)+ [0:0.025:1]*(sprc(2) - sprc(1))];
xi_shift = 20:2:380;
contourf(xi_shift,yi,abs(pvclim_shift)',[-1,cl,1],'linecolor','flat') 
hold on
caxis([sprc(1) sprc(2)]);
gebco('k')
c1 = colorbar;
...
colormap(ax5,cmap)
va = axis;
axis([va(1:2),2000,2019.5])
%xlim([0 10000])
hold on
plot(va(1:2),[2004,2004],'k-')
plot(va(1:2),[2016,2016],'k-')
c4 = colorbar('FontSize',9,'Location','southoutside');
...
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/NPac_hovmoller_PV.png')
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/NPac_hovmoller_PV.fig')
