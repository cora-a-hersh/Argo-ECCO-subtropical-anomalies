...
if ~exist('sa_fwa')
    ...
    a = matfile('gridonSigma1_anomalies_Argo_CORA_sla_superobs_Sep2022_1998_2020.mat');
...
pv_anom(:,a_yi<0,:) = -pv_anom(:,a_yi<0,:); %multiply southern hemisphere by -1

pvclim = pvclim(1:4:end,21:4:621); %make same size as argo anomalies
pvclim(:,a_yi<0,:) = -pvclim(:,a_yi<0,:); %multiply southern hemisphere by -1
%mask outside winter outcrop
...
zpe(ypath<0,:) = -zpe(ypath<0,:); %multiply southern hemisphere by -1

zpe_noseas = NaN(size(zpe));
for ll = 1:size(zpe,1) %remove seasonal cycle to match argo
    if sum(isnan(squeeze(zpe(ll,:)))) > 295
        continue
    end
    [seasonal_anom,seasonal_mean,seasonal_coef,seasonal_m] = seasonal(e.yrgrid,squeeze(zpe(ll,:)));
    zpe_noseas(ll,:) = zpe(ll,:) - seasonal_mean';
end
ecco_pv_data = zpe_noseas;
...
xticks([120 140 160 180 200 220 240 260])
...
plot(xpath,ypath,'r-','LineWidth',2) %ecco streamline
...
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/SPac_hovmoller_spice.png')
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/SPac_hovmoller_spice.fig')
...
ax1 = subaxis(1,3,1,'SH',0.03);
[sprc]=prctile(abs(pvclim(:)'),[5,95]);
cl = [sprc(1)+ [0:0.025:1]*(sprc(2) - sprc(1))];
contourf(xi,yi,abs(pvclim)',[-1,cl,1],'linecolor','flat') %plot absolute value of pv climatology
hold on
caxis([sprc(1) sprc(2)]);
gebco('k')
c1 = colorbar;
...
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/SPac_hovmoller_PV.png')
%saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/SPac_hovmoller_PV.fig')
