...
sigma1 =  '28.95'; %WHICH SIGMA LEVEL DO YOU WANT TO LOOK AT?
...
ii = 52; %WHICH STREAMLINE DO YOU WANT TO LOOK AT?
maxlag = 96; %months
...
for pp = 1:length(lon_nums) %for each point along the streamline
    ...
    e_R(:,pp) = r;
    e_Lags(:,pp) = lags;
    e_corrpeak(pp) = max(r(lags <= 0 & lags >= -60)); %% make sure lag is negative
...
cmocean('balance',20)
vax = axis;
text(vax(1)+10,vax(4)-1000,'Argo/spice')
...
cmocean('balance',20)
vax = axis;
text(vax(1)+10,vax(4)-1000,'ECCO/spice');
...
cmocean('balance',20)
vax = axis;
text(vax(1)+10,vax(4)-1000,'Argo/PV')
...
cmocean('balance',20)
vax = axis;
text(vax(1)+10,vax(4)-1000,'ECCO/PV')

saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/SPac_downstream_correlation.png');
saveas(gcf,'/atalanta/home/chersh/SpiceAnomalies/Paper1_figures/SPac_downstream_correlation.fig');
