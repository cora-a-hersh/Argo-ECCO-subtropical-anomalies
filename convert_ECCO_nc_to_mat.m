...
%THIS IS AN OLDER VERSION -- USE VERSION IN
%/atalanta/home/chersh/SpiceAnomalies/ECCO_experiments
...
if exist([rootdir,'/ECCOv4r4/exps/',expname,'/run/regularpoles_spline/NVELMASS'],'dir')
    vel_avail = 1;
else
    vel_avail = 0;
end
...
%{
datdir = ['/vast/ECCOv4r4/exps/',expname,'/run/regularpoles/'];
savedatdir = ['/vast/ECCOv4r4/exps/',expname,'/run/regularpoles/mat/'];
%}
datdir = [rootdir,'/ECCOv4r4/exps/',expname,'/run/regularpoles_spline'];
savedatdir = [rootdir,'/ECCOv4r4/exps/',expname,'/run/regularpoles_spline/mat/'];

tname = [datdir,'/THETA/THETA_on_sigma1_2000_01.nc'];
nyears = 2017-1992 +1;  
...
readme = 'made from J Gebbie & C Hersh run of MITgcm for eccov4r4 by convert_ECCO_nc_to_mat.m  Dec 2023';
...
varnames = {'pri','pti','si','drhodri','ui','vi','dnum','yrgrid'};
...
tname = [datdir,'THETA/THETA_2000_01.nc'];
x = ncread(tname,'lon');
x = x(:);
...
for iyear = 1993:2017
    tic
    cyear = int2str(iyear)
    [si,pdi,pti,vi,ui]=deal(NaN(nx,ny,nz,12)); % cannot do all years in one go! Not enough memory on atalanta: do in 12 months blocks
    [mxi,fwi,qi,txi,tyi] = deal(NaN(nx,ny,12));
...
readme = 'made from J Gebbie and C Hersh run of MITgcm for eccov4r4 by convert_ECCO_nc_to_mat.m Dec 2023';

savename = [savedatdir,'P_gridded/','gridonP_ecco4r4_',expname,'_',cyear,'.mat']

save -v7.3 junk.mat xi yi zi yrgrid si pdi pti mxi fwi qi txi tyi ui vi dnum yrgrid readme
...
varnames = {'pdi','pti','si','ui','vi','mxi','txi','tyi','qi','fwi','dnum','yrgrid'};
